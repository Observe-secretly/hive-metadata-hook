        ...
        for (String message : messages) {
            if(StringUtils.isNotBlank(message)) {
                ProducerRecord<String, String> record =
                        new ProducerRecord<String, String>(ConfProperties.HOOK_TOPIC.value.toString(), message);

                if (LOG.isDebugEnabled()) {
                    LOG.debug("Sending message for topic {}: {}", ConfProperties.HOOK_TOPIC.value.toString(), message);
                }
